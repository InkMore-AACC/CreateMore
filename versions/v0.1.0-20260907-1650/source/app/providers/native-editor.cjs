...
function createNativeEditor({BrowserWindow,Menu,parent,url,bundle,onSave,confirmReplace}={}){
  if(!BrowserWindow)throw new Error('BrowserWindow is required');const base=endpoint(url,'http://127.0.0.1:8188');const origin=new URL(base).origin;let window=null;let ready=null;let savedSignature=null;let nextBundle=bundle;let importing=false;let disposing=false;
  const signature=b=>JSON.stringify({nodes:b.gui?.nodes,links:b.gui?.links,groups:b.gui?.groups,definitions:b.gui?.definitions});
  ...
  async function read(){await open();const value=await native('const result=await app.graphToPrompt();if(!result?.workflow?.nodes||!result.output)throw new Error("原生前端尚未返回完整转换结果");return {gui:result.workflow,api:result.output,selectedNodeIds:Object.keys(app.canvas?.selected_nodes||{})};');return value;}
  ...
  async function convert(gui,options){await open();return importGui(gui,options);}
