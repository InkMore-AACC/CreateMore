...
async function main(){
  ...
  await compare('app',path.join(destination,'resources','app','app'));await compare('resources',path.join(destination,'resources'));await compare('examples',path.join(destination,'examples'));
  ...
  const projectDir=path.join(destination,'examples','实机验收工程'),store=new ProjectStore({dataDir:path.join(root,'.test-output','release-readonly-check')}),project=JSON.parse(await fs.readFile(path.join(projectDir,'.createmore.json'),'utf8'));let assets=0,nodes=0;
  ...
  const evidence={at:new Date().toISOString(),release:destination,exeHash:release.sha256,files:files.length,applicationHash:hash(JSON.stringify(files)),workflows:catalog.length,example:{nodes,assets},verified:true,scope:'只读逐字节对照app/resources/examples；实际启动和功能运行由Electron验收另记'};
