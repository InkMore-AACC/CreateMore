...
function boot(snapshot){
  ...
  const context={window,document,CanvasModel:Model,innerWidth:1440,innerHeight:1000,CustomEvent:window.CustomEvent,localStorage:{getItem:()=>JSON.stringify(snapshot),setItem(){}},setTimeout:()=>1,clearTimeout(){},setInterval:()=>1,clearInterval(){},Date,Blob,URL,console};
...
test('parameter UI removes inactive switches and false status while counting only image generation',()=>{const ui=boot(state([normal('video','video',{source:'本地 ComfyUI',count:99,quality:'4K'}),normal('text','text',{source:'Codex'}),normal('audio','audio',{source:'本地 ComfyUI'}),normal('image','image',{source:'本地 ComfyUI',count:3,quality:'4K'}),normal('image2','image',{source:'Codex Image2',count:2,ratio:'1:1'})]));assert.equal(ui.document.querySelector('[data-prop="web"]'),null);assert.equal(ui.document.querySelector('[data-prop="autoLink"]'),null);assert(!ui.document.querySelector('#nodes').textContent.includes('尚未连接'));assert.equal(ui.document.querySelector('.node[data-id="video"] [data-action="spec"]'),null);assert.equal(ui.document.querySelector('.node[data-id="image"] [data-action="spec"]').textContent,'3 张');ui.click('.node[data-id="image"] [data-action="spec"]');assert.equal(ui.document.querySelector('[name="quality"]'),null);assert.equal(ui.document.querySelector('[name="ratio"]'),null);ui.click('[data-action="close-panel"]');ui.click('.node[data-id="image2"] [data-action="spec"]');assert(ui.document.querySelector('[name="ratio"]'));assert.match(ui.document.querySelector('#panel-body').textContent,/实际像素尺寸以返回图片为准/);});
