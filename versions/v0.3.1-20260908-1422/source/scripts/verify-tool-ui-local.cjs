...
protocol.registerSchemesAsPrivileged([{scheme:'createmore-media',privileges:{standard:true,secure:true,supportFetchAPI:true,stream:true}}]);
...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    const records=(await service.resources.list('tools',{source:'builtin'})).filter(r=>!only||only.includes(r.name));
    ...
    report.ok=report.cases.length===records.length&&report.cases.every(c=>c.ok);report.summary={localOutputs:report.cases.filter(c=>c.status==='ui-local-output-verified').length,specializedBlocked:report.cases.filter(c=>c.status==='blocked-unverified-specialized').length,failed:report.cases.filter(c=>c.status==='failed').length,artisticAcceptance:false};await fs.writeFile(path.join(base,'final-ui.png'),(await win.webContents.capturePage()).toPNG());
