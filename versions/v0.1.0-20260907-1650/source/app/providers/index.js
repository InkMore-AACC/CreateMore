...
class ProviderHub{
  ...
  async run(snapshot,options={}){const fixed=await this.prepare(snapshot),provider=await this.providerFor(fixed.provider,fixed.connection);return fixed.provider==='comfyui'?require('./comfy-lane.cjs').inComfyLane(provider,()=>provider.run(fixed,options),options):provider.run(fixed,options);}
