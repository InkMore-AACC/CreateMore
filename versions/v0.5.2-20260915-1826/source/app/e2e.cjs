...
async function run({ window, service, appDir, dataDir }) {
  ...
  try {
    ...
    await caseRun('generated image card hides per-card workflows and global settings isolate automatic input routes',async()=>{
      const node=await create('image');await editNode(node.id,{source:'本地 ComfyUI'});await save();
      assert.equal(await evaluate(id=>!!document.querySelector('.node[data-id="'+id+'"] .local-mapping-access, .node[data-id="'+id+'"] [data-workflow-choice]'),node.id),false);
      await evaluate(()=>window.CreateMoreDesktop.handlers['node-config']({dataset:{type:'image'}}));await waitDOM('[data-desktop-form="binding"]');
      const controls=await evaluate(()=>({text:!!document.querySelector('[name="binding-text"]'),single:!!document.querySelector('[name="binding-singleImage"]'),multi:!!document.querySelector('[name="binding-multiImage"]'),body:document.querySelector('#panel-body').textContent}));
      assert.ok(controls.text&&controls.single&&controls.multi);assert.doesNotMatch(controls.body,/UltraSharp|尺寸调整|图片工具 · 裁切/);
      return {nodeId:node.id,perCardWorkflowHidden:true,automaticRoutes:['text','singleImage','multiImage'],toolWorkflowsExcluded:true};
    });
    ...
    await caseRun('remote public parameter form keeps exact types and defaults through real canvas IPC and disk save', async () => {
      ...
      try {
        const executionNode = await create('image');
        await closePanel(); await select([executionNode.id]); await click(`[data-action="execution-choose"][data-node="${executionNode.id}"]`); await waitDOM('[data-action="execution-use"][data-id="e2e-offer"]');
        ...
        const loaded = await service.store.loadCanvas(service.current.projectDir, service.current.id), node = loaded.state.nodes.find(n => n.id === executionNode.id);
    ...
    await caseRun('selected composer stays fixed below its card and may overflow without moving the canvas',async()=>{
      await closePanel();
      const sizes=[];
      for(const zoom of [.5,1]){
        await evaluate(k=>{const C=window.CreateMoreCanvas,n=C.state().nodes.find(n=>n.type==='image'&&C.isGenerator(n));if(!n)throw new Error('缺少生成图片卡片');C.ui.selected=new Set([n.id]);C.state().view={x:280-n.x*k,y:120-n.y*k,k};C.render();},zoom);
        await evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));
        const bounds=await evaluate(()=>{const p=document.querySelector('.node.selected .node-params').getBoundingClientRect(),n=document.querySelector('.node.selected .node-face').getBoundingClientRect(),view={...window.CreateMoreCanvas.state().view};return {width:p.width,height:p.height,left:p.left,right:p.right,top:p.top,bottom:p.bottom,nodeBottom:n.bottom,view};});
        assert.ok(bounds.width>=300*zoom,'编辑面板宽度异常');assert.ok(bounds.top>=bounds.nodeBottom,'编辑面板没有固定在卡片下方');assert.equal(bounds.view.k,zoom,'渲染控件不应改变画布缩放');sizes.push({zoom,...bounds});
      ...
      assert.ok(Math.abs(sizes[1].width-sizes[0].width)<3,'生成编辑面板应保持固定屏幕宽度，不随画布缩放');assert.ok(Math.abs((sizes[1].top-sizes[1].nodeBottom)-(sizes[0].top-sizes[0].nodeBottom))<3,'生成编辑面板与卡片的屏幕间距应固定');return {sizes,fixedScreenSize:true,overflowAllowed:true};
    });
