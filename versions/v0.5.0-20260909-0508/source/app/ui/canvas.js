...
(function () {
  'use strict';
  const {Graph,types,seed,blankShot,clone,ports,portFor,portPoint,nodeHeight,inViewport,isGenerator,mediaBox,ratioValue}=CanvasModel;
  ...
  function nodeContent(n){
    ...
    if(safeAsset(n.asset)&&['image','video','audio','custom'].includes(n.type)){
      ...
      if(kind==='image')return '<img class="media" draggable="false" loading="lazy" decoding="async"'+media+' alt="'+esc(n.title)+'">'+(n.asset.startsWith('assets/')?'<span class="media-note">构图示意</span>':'')+((n.outputAssets?.length||0)>1?'<span class="media-count">'+icon('image')+' '+n.outputAssets.length+' 张</span>':'');
    }
  ...
  function nodeParams(n){
    ...
    let header='';
    if(n.type==='image')header=btn('reference-mode','添加参考','plus',attr)+btn('tool','标记','map',attr+' data-tool="标记"')+btn('library-styles','风格','spark')+btn('tool','聚焦','scissors',attr+' data-tool="聚焦"');
    ...
    const spec=n.type==='image'?btn('spec',esc(n.ratio||'16:9')+' · '+esc(n.quality||'2K')+' · '+esc(n.count??1)+'张',null,attr):'<span class="badge">'+(['video','audio','custom'].includes(n.type)?'按工作流参数':'参数见配置')+'</span>';
    return '<div class="node-params '+(n.composerExpanded?'composer-expanded':'')+'" data-owner="'+n.id+'"><div class="params-header">'+header+'<span class="grow"></span>'+btn('composer-expand','', 'expand',attr+' title="扩大编辑框"')+'</div>'+inputStrip+'<textarea aria-label="'+esc(n.title)+' 提示词" data-prop="prompt" placeholder="描述你的创作内容，输入 @ 引用素材">'+esc(n.prompt)+'</textarea><div class="params-options"><select aria-label="生成来源" data-prop="source">'+sources(n).map(s=>'<option value="'+esc(s)+'"'+(n.source===s?' selected':'')+'>'+esc(s==='外部 API'?'API':s)+'</option>').join('')+'</select>'+spec+'<span class="grow"></span>'+btn('generation','', 'up',attr+' class="generate-button" title="提交生成，状态以来源回执为准"')+'</div></div>';
  }
  ...
  function bindMediaAspect(el,n){
    ...
    const apply=()=>{const w=el.naturalWidth||el.videoWidth,h=el.naturalHeight||el.videoHeight;if(!(w>0&&h>0))return;const ratio=w/h;if(Math.abs((n.w/n.h)-ratio)<.01&&n.mediaAspect===ratio)return;const box=mediaBox(ratio);n.w=box.w;n.h=box.h;n.mediaAspect=ratio;dirty();render();};
  ...
  function create(type,source=ui.source,options={}){
    ...
    try{const n=graph.create(type,x,y,source,{...(source===ui.source?{outputId:ui.sourceOutputId}:{}),...options});ui.source=null;ui.sourceOutputId=undefined;ui.selected=new Set([n.id]);dirty();closeTransient();render();return n;}catch(e){toast(e.message);}
  }
  ...
  function action(name,b,event){
    ...
    switch(name){
      ...
      case 'spec':if(n?.type!=='image'){toast('此类型的时长、尺寸等使用当前工作流公开参数。');break;}ui.context=id;panel('图片比例、分辨率与数量','<form id="spec-form"><div class="spec-section"><strong>画面比例</strong><div class="choice-grid">'+['1:1','9:16','16:9','3:4','4:3','3:2','2:3','21:9'].map(v=>'<label><input type="radio" name="ratio" value="'+v+'"'+(n.ratio===v?' checked':'')+'><span>'+v+'</span></label>').join('')+'</div><strong>自定义比例</strong><div class="custom-ratio"><label>宽 <input type="number" name="customWidth" min="1" placeholder="输入宽"></label><span>:</span><label>高 <input type="number" name="customHeight" min="1" placeholder="输入高"></label></div></div><div class="spec-section"><strong>分辨率</strong><div class="choice-grid four">'+['1K','2K','3K','4K'].map(v=>'<label><input type="radio" name="quality" value="'+v+'"'+((n.quality||'2K')===v?' checked':'')+'><span>'+v+'</span></label>').join('')+'</div></div><div class="spec-section"><strong>生成数量</strong><div class="choice-grid three">'+[1,2,4].map(v=>'<label><input type="radio" name="count" value="'+v+'"'+(Number(n.count||1)===v?' checked':'')+'><span>'+v+'张</span></label>').join('')+'</div></div><div class="panel-footer">'+btn('cancel-config','取消')+'<button class="primary" type="submit">确定</button></div></form>','spec');break;
  ...
  document.addEventListener('input',event=>{
    ...
    if(field.dataset.prop==='prompt'&&field.value.endsWith('@')){const owner=field.closest('[data-owner]')?.dataset.owner,refs=state().edges.filter(e=>e.to===owner).map(e=>graph.get(e.from)).filter(Boolean),r=field.getBoundingClientRect();showMenu('<h4>引用已连接的素材</h4>'+(refs.length?refs.map((n,i)=>btn('prompt-reference','@'+(i+1)+' '+esc(n.title),types[n.type].icon,'data-node="'+esc(owner)+'" data-reference="@'+(i+1)+'"')).join(''):'<small style="display:block;padding:8px">请先添加参考素材</small>'),r.left,r.bottom+6);}
  ...
  document.addEventListener('submit',event=>{
    ...
    if(form.id==='spec-form'){const n=graph.get(ui.context),count=Number(values.get('count')),cw=Number(values.get('customWidth')),ch=Number(values.get('customHeight'));if(n?.type!=='image'||![1,2,4].includes(count)){toast('请选择 1、2 或 4 张。');return;}let ratio=String(values.get('ratio')||n.ratio||'1:1');if(cw>0&&ch>0)ratio=cw+':'+ch;if(!Number.isFinite(ratioValue(ratio))){toast('请填写有效的宽高比例。');return;}graph.checkpoint();n.count=count;n.ratio=ratio;n.quality=['1K','2K','3K','4K'].includes(values.get('quality'))?values.get('quality'):'2K';if(!n.asset){const box=mediaBox(ratioValue(ratio));n.w=box.w;n.h=box.h;}}
  ...
  window.CreateMoreCanvas={graph,ui,state,render,renderNodes,renderSide,renderSelection,renderView,updateMediaVisibility,selection,locate,fit,create,panel,closePanel,closeTransient,showMenu,nav,btn,icon,esc,toast,dirty,safeAsset,script,downloadText,action,blankShot,types,clone,isGenerator};
  timer();
})();
