...
const token=mark=>'📌['+mark.name+'](createmore-mark:'+mark.id+')';
const activeMarkIds=prompt=>new Set([...String(prompt||'').matchAll(/\(createmore-mark:([a-zA-Z0-9_-]+)\)/g)].map(match=>match[1]));
...
const stripTokens=prompt=>String(prompt||'').replace(/\s*📌\[[^\]]+\]\(createmore-mark:[a-zA-Z0-9_-]+\)\s*/g,' ').replace(/\s{2,}/g,' ').trim();
...
function expandPrompt(prompt,node){
  ...
  return String(prompt||'').replace(/📌\[([^\]]+)\]\(createmore-mark:([a-zA-Z0-9_-]+)\)/g,(whole,label,markId)=>{const mark=marks.get(markId);if(!mark)return label;const b=mark.bbox;return `图钉标记“${mark.name}”（约位于原图归一化范围 x=${b.x.toFixed(3)}, y=${b.y.toFixed(3)}, width=${b.width.toFixed(3)}, height=${b.height.toFixed(3)}）`;});
}
