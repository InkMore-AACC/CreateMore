  ...
  return service.mutate(async () => {
    ...
    if (kind === 'presets') {
      ...
      for (const asset of imported) {
        ...
        created.push(service.newNode(type, { title: resource.name + ' · ' + asset.title, assetId: asset.id, asset: asset.asset, content: '角色资料', cardKind: 'asset', x: 120 + index++ * 420, y: 160, characterSource: { id: resource.id, revision: resource.revision } }, canvas));
      }
...
async function applyHistory(service, { taskId } = {}) {
  ...
  return service.mutate(async () => {
    ...
    for (const asset of prepared) {
      const type = ['image', 'video', 'audio', 'text'].includes(asset.type) ? asset.type : 'custom';
      const node = service.newNode(type, { title: (source?.title || '历史结果') + ' · 取用', assetId: asset.id, asset: asset.asset, content: type === 'text' && record.text ? record.text : '历史素材', cardKind: 'asset', historySource: { taskId: record.taskId, assetId: asset.id }, x: (source?.x || 120) + (source?.w || 300) + 100, y: (source?.y || 160) + created.length * 300 }, canvas);
      created.push(node);
    }
    if (record.text && !prepared.some(asset => asset.type === 'text')) created.push(service.newNode('text', { title: (source?.title || '历史结果') + ' · 取用', content: record.text, cardKind: 'asset', historySource: { taskId: record.taskId }, x: (source?.x || 120) + (source?.w || 300) + 100, y: (source?.y || 160) + created.length * 300 }, canvas));
