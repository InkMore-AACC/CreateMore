(function (root) {
  ...
  const typeTable = {
    image: {name:'图片', icon:'image', w:380, h:254, sources:['Codex Image2','本地 ComfyUI','RunningHub']},
    video: {name:'视频', icon:'video', w:380, h:214, sources:['本地 ComfyUI','RunningHub']},
  ...
  function node(id, type, x, y, overrides) {
    ...
    return Object.assign({id,type,x,y,w:t.w,h:t.h,title:t.name,owner:'me',prompt:'',content:'',source:t.sources[0],ratio:'16:9',count:'1',quality:'2K',shots:type==='script'?[blankShot()]:[]}, overrides);
  }
  function seed() {
    return {
      version:5, assets:[
  ...
  class Graph {
    ...
    create(type,x,y,from,options={}){
      ...
      const next=this.state.seq+1,n=node(uid('n'),type,x,y,{title:types[type].name+' '+next});
  ...
  const api={Graph,types,seed,blankShot,clone,uid,ports,portFor,portPoint,nodeHeight,mediaType,inViewport};
