...
async function main(){
  ...
  for(const output of outputs){
    ...
    if(previous)for(const kind of ['skills','tools','workflows'])await fs.cp(path.join(previous,'resources',kind),path.join(destination,'resources',kind),{recursive:true,force:true,filter:async file=>{if((await fs.lstat(file)).isSymbolicLink())throw new Error('用户资源包含链接，请先检查');return !file.split(path.sep).includes('.trash');}});
    ...
    await fs.mkdir(path.join(destination,'docs'),{recursive:true});for(const file of ['当前执行约定与工作流复核.md','v0.3.0-本轮修复与待确认.md','LibTV交互补测-2026-09-08.md','使用说明.md','实施进度与待确认.md','验收与审核记录.md','本地功能接入清单.md','生成接入验证记录.md','LAN验证记录.md'])await fs.copyFile(path.join(root,'docs',file),path.join(destination,'docs',file));
    await fs.copyFile(path.join(root,'README.md'),path.join(destination,'README.md'));
    const exe=path.join(destination,'CreateMore.exe'),bytes=await fs.readFile(exe);releases.push({directory:destination,exe,sha256:crypto.createHash('sha256').update(bytes).digest('hex'),version:pkg.version,createdAt:new Date().toISOString(),preservedResourcesFrom:previous||null});
  }
