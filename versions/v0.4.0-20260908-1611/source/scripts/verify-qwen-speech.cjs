...
async function main(){
  ...
  const p=new ComfyProvider({...config,url:'http://127.0.0.1:8189'}),report={base,cases:[],ok:false};
  await fs.mkdir(base,{recursive:true});
  try{
    if((await p.status()).ready)throw Error('8189 already occupied; leave it untouched');
