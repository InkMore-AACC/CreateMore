...
test('resolution conversion preserves custom ratio and guarantees focus pixel floor',()=>{const d=Model.imageDimensions('337:191','1K',{minimumPixels:1048576});assert(Math.abs(d.width/d.height-337/191)<.01);assert(d.pixels>=1048576);assert.equal(d.width%8,0);assert.equal(d.height%8,0);assert.equal(Model.imageDimensions('9:16','4K').height,4096);});
...
test('focus cards lock the crop ratio and direct marking uses synchronized picture and prompt affordances',()=>{const uri='createmore-media://asset/test',node=normal('focus','image',{asset:uri,assetId:'asset',cardKind:'generator',ratio:'337:191',customWidth:337,customHeight:191,focusRegion:{x:1,y:2,cropWidth:337,cropHeight:191},objectMarks:[{id:'mark-one',name:'红色杯子',x:.5,y:.5,bbox:{x:.35,y:.25,width:.3,height:.5}}],prompt:'修改 {{createmore_object:mark-one}}'}),ui=boot(state([node]));assert(ui.document.querySelector('.object-mark-box'));assert.match(ui.document.querySelector('.prompt-object-chip').textContent,/红色杯子/);assert.equal(ui.document.querySelector('.prompt-editor').textContent.includes('createmore_object'),false);assert(ui.document.querySelector('.object-mark-remove'));ui.click('.node[data-id="focus"] [data-action="spec"]');assert(ui.document.querySelector('.custom-ratio.locked'));assert.match(ui.document.querySelector('#panel-body').textContent,/固定遵循框选比例/);ui.click('[data-action="close-panel"]');ui.click('.node[data-id="focus"] [data-action="object-mark-mode"]');assert(ui.document.body.classList.contains('object-mark-picking'));assert.equal(ui.document.querySelector('#panel').hidden,true);});
test('prompt at-menu lists connected images only and numbers them in image order',()=>{const target=normal('target','image',{cardKind:'generator'}),picture=normal('picture','image',{asset:'createmore-media://asset/a',assetId:'a',cardKind:'asset'}),words=normal('words','text',{content:'说明',cardKind:'asset'}),snapshot=state([picture,words,target]);snapshot.edges=[{id:'e1',from:'words',to:'target'},{id:'e2',from:'picture',to:'target'}];const ui=boot(snapshot),field=ui.document.querySelector('.node[data-id="target"] [data-prop="prompt"]');field.textContent='请参考@';ui.send(field,'input');assert.match(ui.document.querySelector('#menu').textContent,/引用已连接的图片/);assert.match(ui.document.querySelector('#menu').textContent,/@1 picture/);assert.doesNotMatch(ui.document.querySelector('#menu').textContent,/words/);});
test('custom ratio is one radio choice and outside click applies it without a confirm button',()=>{const node=normal('image','image',{cardKind:'generator',ratio:'16:9',quality:'2K',count:1}),ui=boot(state([node]));ui.click('.node[data-id="image"] [data-action="spec"]');const form=ui.document.querySelector('#spec-form'),custom=form.querySelector('[name="ratio"][value="custom"]'),preset=form.querySelector('[name="ratio"][value="16:9"]'),width=form.querySelector('[name="customWidth"]'),height=form.querySelector('[name="customHeight"]');assert(custom&&preset);assert.equal(form.querySelector('[type="submit"]'),null);width.value='588';height.value='452';ui.send(width,'input');ui.send(height,'input');assert.equal(custom.checked,true);assert.notEqual(preset.checked,true);ui.click('#viewport');assert.equal(ui.document.querySelector('#panel').hidden,true);assert.equal(ui.app.graph.get('image').ratio,'588:452');assert.equal(ui.app.graph.get('image').customWidth,588);});
test('composer expand opens a separate resizable top-layer and backdrop closes it',()=>{const node=normal('image','image',{cardKind:'generator',prompt:'一只猫'}),ui=boot(state([node]));ui.click('.node[data-id="image"] [data-action="composer-expand"]');const backdrop=ui.document.querySelector('#composer-backdrop');assert.equal(backdrop.hidden,false);assert(ui.document.querySelector('#composer-overlay .composer-overlay-params'));assert.match(ui.document.querySelector('#composer-overlay .prompt-editor').textContent,/一只猫/);assert(ui.document.querySelector('.node[data-id="image"] .node-params'));ui.send(backdrop,'click');assert.equal(backdrop.hidden,true);});
test('nested panels expose a real back action distinct from closing the panel',()=>{const ui=boot(state([]));ui.app.panel('设置首页','第一层','settings');ui.app.panel('识别配置','第二层','object-mark');assert.equal(ui.document.querySelector('#panel-back').hidden,false);ui.click('#panel-back');assert.equal(ui.document.querySelector('#panel-title').textContent,'设置首页');assert.equal(ui.document.querySelector('#panel').hidden,false);assert.equal(ui.document.querySelector('#panel-back').hidden,true);});
