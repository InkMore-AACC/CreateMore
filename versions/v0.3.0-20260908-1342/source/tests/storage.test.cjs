...
test('a renamed missing canvas can recover its specifically named backup',async t=>{const {store,project,canvas}=await setup(t);const original=path.join(canvas.canvasDir,'画布.createmore'),renamed=path.join(canvas.canvasDir,'设计方案.createmore');await fs.rename(original,renamed);await store.saveCanvas(project.projectDir,canvas.id,state());await fs.unlink(renamed);const result=await store.loadCanvas(project.projectDir,canvas.id);assert.equal(result.recovered,true);assert.equal(result.state.nodes[0].content,'initial');});
...
test('atomic save keeps prior state and recovers a corrupt main file', async t => {
  ...
  await fs.writeFile(path.join(canvas.canvasDir, '画布.createmore'), '{broken', 'utf8');
  ...
  await store.saveCanvas(project.projectDir, canvas.id, next);
  assert.equal(JSON.parse(await fs.readFile(path.join(canvas.canvasDir, '备份', '画布.createmore.previous'), 'utf8')).state.nodes[0].content, 'initial');
});
...
test('autosaves only changed state, rotates declared retention, leaves media untouched', async t => {
  ...
  const before = await fs.readFile(path.join(canvas.canvasDir, '画布.createmore'), 'utf8');
  ...
  assert.equal(await fs.readFile(path.join(canvas.canvasDir, '画布.createmore'), 'utf8'), before);
});
...
test('symlink or junction project/media boundaries are rejected', async t => {
  ...
  const manifest = JSON.parse(await fs.readFile(path.join(project.projectDir, '项目.createmore'), 'utf8'));
  manifest.canvases[0].directory = '../other'; await atomicJSON(path.join(project.projectDir, '项目.createmore'), manifest, { backup: false });
