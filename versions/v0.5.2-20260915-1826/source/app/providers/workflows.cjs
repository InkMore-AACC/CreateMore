...
function bindInputs(bundle, snapshot, uploaded = []) {
  ...
  for (const input of bundle.mapping.inputs) {
    ...
    if(input.type==='boolean'){if(value===true||value==='true')value=true;else if(value===false||value==='false')value=false;else throw new ProviderError(`布尔参数无效：${input.label||input.id}`,'PARAMETER_TYPE');}
