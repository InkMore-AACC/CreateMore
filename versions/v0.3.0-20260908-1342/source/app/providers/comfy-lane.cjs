...
async function inComfyLane(provider,work,{signal,onProgress=()=>{}}={}){
  ...
  }finally{if(startedWork&&provider.config?.memoryPolicy==='release-idle'){try{await provider.releaseMemory();}catch(error){try{onProgress({message:'空闲显存释放未执行：'+error.message});}catch{}}}release();if(lanes.get(key)===tail)tail.finally(()=>{if(lanes.get(key)===tail)lanes.delete(key);});}
}
