...
async function main(){
  ...
  for(const output of outputs){
    ...
    await fs.mkdir(path.join(destination,'docs'),{recursive:true});for(const file of ['当前执行约定与工作流复核.md','本地专用工作流实施记录-20260908.md','v0.3.0-本轮修复与待确认.md','LibTV交互补测-2026-09-08.md','使用说明.md','实施进度与待确认.md','验收与审核记录.md','本地功能接入清单.md','生成接入验证记录.md','LAN验证记录.md'])await fs.copyFile(path.join(root,'docs',file),path.join(destination,'docs',file));
