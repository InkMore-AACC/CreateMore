...
async function run({ window, service, appDir, dataDir }) {
  ...
  try {
    ...
    toolResourceId = (await service.resources.save('tools', { name: 'E2E 高清', handler: '高清', mediaType: 'image', provider: 'comfyui', enabled: true, order: 0 })).id;
    service.hub = {
    ...
    await caseRun('new project form through real IPC', async () => {
      ...
      const manifest = JSON.parse(await fs.readFile(path.join(projectDir, '.createmore.json'), 'utf8'));
      assert.equal(manifest.name, 'E2E 项目'); assert.equal(manifest.canvases.length, 1);
      return { manifest: path.join(projectDir, '.createmore.json'), canvasId: firstCanvasId };
    });
    ...
    await caseRun('node button, prompt input and save to disk', async () => {
      ...
      const disk = JSON.parse(await fs.readFile(path.join(firstCanvasDir, 'canvas.json'), 'utf8'));
      ...
      return { nodeId: node.id, prompt: node.prompt, owner: node.owner, file: path.join(firstCanvasDir, 'canvas.json') };
    });
    await caseRun('actual PNG import, external reference and media protocol', async () => {
      ...
      await save();
      const raw = await fs.readFile(path.join(firstCanvasDir, 'canvas.json'), 'utf8'); assert(!raw.includes('createmore-media:'));
    ...
    await caseRun('remote public parameter form keeps exact types and defaults through real canvas IPC and disk save', async () => {
      ...
      try {
        ...
        return { nodeId: node.id, publicFields: 4, parameters: node.execution.parameters, actualDOMEvents: true, actualCanvasSaveIPC: true, diskFile: path.join(service.current.canvasDir, 'canvas.json'), capabilityDiscovery: 'isolated mock offers only; no multi-PC claim', newGenerationTasks: 0 };
