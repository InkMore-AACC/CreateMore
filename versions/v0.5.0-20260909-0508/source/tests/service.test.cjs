...
const { CreateMoreService, configuredBinding, defaultCardBindings } = require('../app/core/service.cjs');
...
test('local generation workflow routing is isolated by card type and connected material count', () => {
  const target={id:'target',type:'image'},c={state:{nodes:[target,{id:'a',type:'image'},{id:'b',type:'image'},{id:'tool',type:'custom',outputType:'audio'}],edges:[]}},settings={cardBindings:structuredClone(defaultCardBindings),bindings:{}};
  assert.deepEqual(configuredBinding(settings,target,c),{binding:'default:image-zimage',route:'text'});
  c.state.edges.push({from:'a',to:'target'});assert.deepEqual(configuredBinding(settings,target,c),{binding:'default:image-qwen-edit-1',route:'singleImage'});
  c.state.edges.push({from:'b',to:'target'});assert.deepEqual(configuredBinding(settings,target,c),{binding:'default:image-qwen-edit-2',route:'multiImage'});
  target.workflowId='default:explicit';assert.deepEqual(configuredBinding(settings,target,c),{binding:'default:image-qwen-edit-2',route:'multiImage'});
  target.toolResource='聚焦';assert.deepEqual(configuredBinding(settings,target,c),{binding:'default:explicit',route:'multiImage'});
});
...
test('external and copied assets render through capability URLs but persist actual paths', async t => {
  ...
  let stored = JSON.parse(await fs.readFile(path.join(service.current.canvasDir, '画布.createmore'), 'utf8'));
  ...
  stored = JSON.parse(await fs.readFile(path.join(service.current.canvasDir, '画布.createmore'), 'utf8'));
...
test('mock-provider image result is registered, persisted and associated with real history', async t => {
  ...
  const disk = JSON.parse(await fs.readFile(path.join(service.current.canvasDir, '画布.createmore'), 'utf8')); assert.equal(disk.state.nodes[0].assetId, asset.id);
});
...
test('result association retry after save failure must persist node instead of trusting history entry', async t => {
  ...
  const disk = JSON.parse(await fs.readFile(path.join(service.current.canvasDir, '画布.createmore'), 'utf8'));
...
test('persisted pending result survives an actual service restart after canvas save failure', async t => {
  ...
  const disk = JSON.parse(await fs.readFile(path.join(restarted.current.canvasDir, '画布.createmore'), 'utf8'));
...
test('script conversion tool creates one editable storyboard and retains original text and output',async t=>{
  ...
  const script=service.current.state.nodes.find(n=>n.type==='script');assert.ok(script);assert.equal(script.originTaskId,task.id);assert.equal(script.source,'Codex');assert.equal(script.shots[0].description,rows[0].description);assert.equal(script.shots[0].camera,'缓慢推进');assert.equal(script.sourceNodeId,source.id);assert.equal(JSON.stringify(source),before);
...
test('local storyboard card uses its isolated global binding and preserves JSON output contract',async t=>{
  const {service}=await setup(t);const node=await add(service,'script');service.settings.cardBindings.script.text='default:chosen-text';service.workflowRead=async key=>({id:key,api:{},mapping:{inputs:[],outputs:[]}});const snapshot=await service.buildSnapshot(node.id,{provider:'comfyui'});assert.equal(snapshot.workflowId,'default:chosen-text');assert.equal(snapshot.workflow.id,'default:chosen-text');assert.equal(snapshot.workflowRoute,'text');assert.equal(snapshot.kind,'text');assert.match(snapshot.prompt,/仅返回JSON数组/);
});
