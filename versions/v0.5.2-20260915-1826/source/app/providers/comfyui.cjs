...
const { spawn, ChildProcess, execFile } = require('node:child_process');
...
// Comfy V3 autogrow inputs are flattened (images.image0), not an `images` socket.
// Expand the live schema just as Comfy's Autogrow._expand_schema_for_dynamic does.
function validationInputs(input = {}) {
  const result={required:{...input.required},optional:{...input.optional}};
  for(const group of ['required','optional'])for(const [key,spec] of Object.entries(result[group])) {
    if(spec?.[0]!=='COMFY_AUTOGROW_V3')continue;
    const template=spec[1]?.template;
    if(!template)continue;
    const names=template.names || (typeof template.prefix==='string'&&Number.isInteger(template.max)&&template.max>=1&&template.max<=100?Array.from({length:template.max},(_,i)=>template.prefix+i):null);
    const required=Object.values(template.input?.required||{}),optional=Object.values(template.input?.optional||{}),leaf=required[0]||optional[0];
    if(!names||!leaf)continue;
    delete result[group][key];
    names.forEach((name,i)=>{result[required.length&&i<(template.min||0)?'required':'optional'][key+'.'+name]=leaf;});
  }
  return result;
}
...
class ComfyProvider {
  ...
  async releaseMemory() {
    if(!['localhost','127.0.0.1','[::1]'].includes(new URL(this.url).hostname))throw new ProviderError('显存释放只操作本机连接，远程设备请由其所有者处理','REMOTE_MEMORY_CONTROL');
    const queue=await this.request('/queue');if(queue.queue_running?.length||queue.queue_pending?.length)throw new ProviderError('ComfyUI 仍有运行或排队任务，未释放模型','BUSY');
    await this.request('/free',post({unload_models:true,free_memory:true}));
    return {state:'requested',message:'后台已接受卸载模型和释放缓存请求；不会停止服务，也不等于显存已经完全释放。'};
  }
  ...
  async stopOwned() { if (!this.owned) return { stopped:false, reason:'不是由 CreateMore 启动的进程' }; const queue=await this.request('/queue'); if (queue.queue_running?.length || queue.queue_pending?.length) throw new ProviderError('后台仍有任务，不能直接关闭','BUSY');const child=this.owned;
    ...
    if(process.platform==='win32'&&child instanceof ChildProcess&&Number.isSafeInteger(child.pid)&&child.pid>0){
      // A Windows venv launcher can have a second Python process. Only this live,
      // application-owned process tree is addressed; never kill by executable name.
      try{await execFileAsync(path.join(process.env.SystemRoot||'C:\\Windows','System32','taskkill.exe'),['/PID',String(child.pid),'/T','/F'],{windowsHide:true,timeout:10000});}
      catch(error){if(child.exitCode==null&&child.signalCode==null)return {stopped:false,reason:'自有进程树尚未确认退出：'+error.message};}
      const status=await this.status(),stopped=(child.exitCode!=null||child.signalCode!=null)&&!status.ready;
      if(stopped&&this.owned===child)this.owned=null;return {stopped,reason:stopped?undefined:'自有进程停止后端口仍可访问，可能有另一实例；未继续终止其他进程'};
    }
  ...
  async validate(api, mapping) {
    const nodes=await this.request('/object_info'); const issues=[];
    for(const output of mapping?.outputs||[]) {
      const type=api[String(output.nodeId)]?.class_type;
      if(type&&nodes[type]?.output_node===false)issues.push({nodeId:String(output.nodeId),input:output.key,reason:'映射指向非结果节点 '+type+'，请选择保存或预览结果节点'});
    }
    for(const [id,node] of Object.entries(api)) { const def=nodes[node.class_type]; if(!def) {issues.push({nodeId:id,reason:`缺少节点 ${node.class_type}`});continue;} const input=validationInputs(def.input); for(const key of Object.keys(input.required))if(!Object.hasOwn(node.inputs,key))issues.push({nodeId:id,input:key,reason:'缺少必需参数'});for(const [key,spec] of Object.entries({...input.required,...input.optional})) { const val=node.inputs[key];const options=Array.isArray(spec?.[0])?spec[0]:spec?.[0]==='COMBO'?spec?.[1]?.options:null;const fileUpload=spec?.[1]?.image_upload||spec?.[1]?.audio_upload||spec?.[1]?.video_upload;if(options && !fileUpload && val!=null && !Array.isArray(val) && !options.includes(val)) issues.push({nodeId:id,input:key,reason:'模型或选项不存在',value:val});if(['INT','FLOAT'].includes(spec?.[0]) && val!=null && !Array.isArray(val) && (typeof val!=='number'||!Number.isFinite(val)||(spec[0]==='INT'&&!Number.isInteger(val))||(spec[1]?.min!=null&&val<spec[1].min)||(spec[1]?.max!=null&&val>spec[1].max)))issues.push({nodeId:id,input:key,reason:'数值类型或范围无效'}); } }
    if(issues.length) throw new ProviderError('工作流与当前 ComfyUI 环境不兼容：'+issues.slice(0,5).map(i=>'节点 '+i.nodeId+(i.input?' / '+i.input:'')+'：'+i.reason+(i.value!=null?'（'+String(i.value).slice(0,160)+'）':'')).join('；')+(issues.length>5?'；另有 '+(issues.length-5)+' 项':''),'ENVIRONMENT_MISMATCH',{issues}); return {valid:true};
  }
  preflight(snapshot,{signal,outputDir}={}) {
  ...
  async run(snapshot,{onProgress=()=>{},signal,outputDir}={}) {
    const bundle=this.preflight(snapshot,{signal,outputDir});
    ...
    const api=bindInputs(bundle,snapshot,uploaded); await this.validate(api,bundle.mapping); const clientId=crypto.randomUUID(); let socket;
