...
async function main() {
  ...
  try {
    ...
    const privateCanvasFile = path.join(device.current.canvasDir,'canvas.json'), privateCanvasHash = await fileHash(privateCanvasFile);
    ...
    const disk = JSON.parse(await fs.readFile(path.join(requester.current.canvasDir,'canvas.json'),'utf8')); assert.equal(disk.state.nodes.find(n=>n.id===target.id).assetId,asset.id);
