...
class CreateMoreService extends EventEmitter {
  ...
  async applyResult(task,result){if(task.snapshot?.remoteExecution)return;result=await inputs.hydrateText(result);return this.mutate(async()=>{
    ...
    else if(task.snapshot.resultMode==='script'){const source=c.state.nodes.find(x=>x.id===task.snapshot.sourceNodeId);let shots;try{const parsed=require('./creative.cjs').parseJSON(result.text);const rows=Array.isArray(parsed)?parsed:parsed.shots;if(!Array.isArray(rows)||!rows.length||rows.length>1000||rows.some(row=>!row||typeof row.description!=='string'||!row.description.trim()))throw new Error('未返回有效分镜行');shots=rows.map(row=>Object.fromEntries(['duration','description','prompt','size','camera','light','dialogue','sound'].map(key=>[key,String(row[key]??'')])));}catch(error){this.log('warn','剧本生成文字已保留，但未解析为分镜表',error,{taskId:task.id});}const next=this.newNode(shots?'script':'text',{title:task.title+(shots?'':' · 格式待整理'),...(shots?{shots}:{}),content:result.text||'模型未返回可解析分镜；请在任务详情核对原始结果',sourceNodeId:source?.id,assetId:outputs[0]?.id,asset:outputs[0]?.asset,outputAssets:outputs.map(a=>a.id),x:(source?.x||100)+(source?.w||380)+140,y:source?.y||150},c);c.state.nodes.push(next);if(source)c.state.edges.push({id:id('e'),from:source.id,to:next.id});}
    else if(task.snapshot.resultMode==='storyboard'&&result.storyboard){const source=c.state.nodes.find(x=>x.id===task.snapshot.sourceNodeId);const next=this.newNode('script',{title:task.title,shots:clone(result.storyboard.shots),sourceNodeId:source?.id,sourceAssetId:result.storyboard.sourceAssetId,analysisVersion:result.storyboard.version,analysisErrors:result.storyboard.errors,content:result.partial?'部分范围未完成':'代表帧分析',x:(source?.x||100)+(source?.w||380)+140,y:source?.y||150},c);c.state.nodes.push(next);if(source)c.state.edges.push({id:id('e'),from:source.id,to:next.id});}
    else if(task.snapshot.resultMode==='new'){const source=c.state.nodes.find(x=>x.id===task.snapshot.sourceNodeId);if(result.text&&!outputs.length){const next=this.newNode('text',{title:task.title,content:result.text,sourceNodeId:source?.id,x:(source?.x||100)+(source?.w||380)+140,y:source?.y||150},c);c.state.nodes.push(next);if(source)c.state.edges.push({id:id('e'),from:source.id,to:next.id});}for(const a of outputs){const content=inputs.isText(a.type,a.path)?await inputs.readText(await this.assetPath(a.id,c)):'处理结果';const next=this.newNode(a.type,{title:task.title,asset:a.asset,assetId:a.id,content,x:(source?.x||100)+(source?.w||380)+140,y:(source?.y||150)+outputs.indexOf(a)*330,sourceNodeId:source?.id},c);c.state.nodes.push(next);if(source)c.state.edges.push({id:id('e'),from:source.id,to:next.id});}}
  ...
  async call(method,args={}){
    if(method==='app.status')return {version:'0.1.0',identity:this.identity,current:await this.view(),settings:redact(this.settings),providers:await this.hub.status(),media:await this.media.status(),lan:await this.collab.status()};
