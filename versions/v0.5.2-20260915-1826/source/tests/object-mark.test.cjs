...
test('object marking stores a stable prompt link and routes the marked current image as its own reference',async t=>{
  ...
  assert.equal(result.mark.name,'青色杯子');assert.match(node.prompt,/\{\{createmore_object:mark-/);assert.doesNotMatch(node.prompt,/📌|青色杯子/);assert.deepEqual(configuredBinding(service.settings,node,service.current),{binding:'default:image-qwen-edit-1',route:'singleImage'});
  const snapshot=await service.buildSnapshot(node.id);assert.equal(snapshot.references.length,1);assert.equal(snapshot.references[0].id,node.assetId);assert.match(snapshot.prompt,/对象引用：名称“青色杯子”/);assert.match(snapshot.prompt,/x=0\.320/);assert.match(snapshot.prompt,/不得绘制定位图钉、虚线框、标签、坐标或说明文字/);assert.doesNotMatch(snapshot.prompt,/createmore_object:/);
});
