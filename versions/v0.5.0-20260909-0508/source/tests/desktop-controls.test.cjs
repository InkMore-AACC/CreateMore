...
function setup(resources = {}) {
  ...
  const C = { esc: escape, btn: (action, label, icon, attrs = '') => '<button data-action="' + action + '" ' + attrs + '>' + label + '</button>', icon: () => '', toast: text => notices.push(text), graph, ui, types, clone, isGenerator: n => n.cardKind !== 'asset', render() {}, dirty() {}, closePanel() {}, nav: () => '', panel(title, html, kind) { panel = html; ui.panelKind = kind; }, };
...
test('image generation settings isolate text single-image and multi-image workflows from tool workflows', async()=>{const h=setup();h.overrides['workflow.catalog']=()=>[{id:'image-zimage',label:'图片生成',kind:'image',version:'1'},{id:'image-qwen-edit-1',label:'1 张参考',kind:'image',version:'1'},{id:'image-qwen-edit-2',label:'2 张参考',kind:'image',version:'1'},{id:'tool-image-upscale',label:'图片工具 · 放大',kind:'image',version:'1'}];h.overrides['settings.get']=()=>({bindings:{image:'default:image-zimage'},cardBindings:{image:{text:'default:image-zimage',singleImage:'default:image-qwen-edit-1',multiImage:'default:image-qwen-edit-2'}}});await h.handlers['node-config']({dataset:{type:'image'}});assert.match(h.panel(),/无素材／文字生成/);assert.match(h.panel(),/1 张图片参考/);assert.match(h.panel(),/多张图片参考/);assert.doesNotMatch(h.panel(),/tool-image-upscale|图片工具 · 放大/);await h.submit('binding',{'binding-text':'default:image-zimage','binding-singleImage':'default:image-qwen-edit-1','binding-multiImage':'default:image-qwen-edit-2'});const saved=h.calls.find(c=>c.method==='settings.save').args;assert.equal(saved.cardBindings.image.multiImage,'default:image-qwen-edit-2');});
