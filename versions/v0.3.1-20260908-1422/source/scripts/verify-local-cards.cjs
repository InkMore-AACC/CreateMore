...
async function main(){
  ...
  finally{const p=service.hub?.providers?.comfyui;if(p?.owned)report.cleanup=await p.stopOwned().catch(e=>({error:e.message}));await save();console.log('REPORT '+path.join(base,'report.json'));await service.close();}
}
