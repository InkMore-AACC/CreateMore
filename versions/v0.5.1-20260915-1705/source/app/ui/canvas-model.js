(function (root) {
  ...
  function mediaBox(ratio,max=420){
    ...
    return value>=1?{w:max,h:Math.max(1,Math.round(max/value))}:{w:Math.max(1,Math.round(max*value)),h:max};
  }
  ...
  const qualityEdges={1:1024,2:2048,3:3072,4:4096};
  function imageDimensions(ratio,quality='2K',{minimumPixels=0,maxEdge=8192}={}){
    const value=typeof ratio==='number'?ratio:ratioValue(ratio),tier=Number(String(quality).match(/[1-4]/)?.[0]||2),edge=qualityEdges[tier]||2048;
    if(!Number.isFinite(value)||value<=0)throw new Error('图片比例无效');
    let width=value>=1?edge:edge*value,height=value>=1?edge/value:edge;
    const pixels=width*height;if(Number.isFinite(minimumPixels)&&minimumPixels>pixels){const scale=Math.sqrt(minimumPixels/pixels);width*=scale;height*=scale;}
    const largest=Math.max(width,height);if(largest>maxEdge){const scale=maxEdge/largest;width*=scale;height*=scale;}
    const align=n=>Math.max(8,Math.round(n/8)*8),alignUp=n=>Math.max(8,Math.ceil(n/8)*8);width=align(width);height=align(width/value);
    if(Number.isFinite(minimumPixels)&&minimumPixels>width*height){const scale=Math.sqrt(minimumPixels/(width*height));width=alignUp(width*scale);height=alignUp(width/value);while(width*height<minimumPixels){width+=8;height=alignUp(width/value);}}
    if((!minimumPixels||minimumPixels<=0)&&height>maxEdge){height=align(maxEdge);width=align(height*value);}
    return {width,height,pixels:width*height,ratio:value,quality:tier+'K'};
  }
  ...
  const api={Graph,types,seed,blankShot,clone,uid,ports,portFor,portPoint,nodeHeight,mediaType,inViewport,inferCardKind,isGenerator,mediaBox,ratioValue,imageDimensions};
