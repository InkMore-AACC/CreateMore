...
async function main(){
  ...
  const config=JSON.parse(await fs.readFile(path.join(process.env.APPDATA,'CreateMore/providers.json'),'utf8')).comfyui,p=new ComfyProvider({...config,url:'http://127.0.0.1:8189'}),report={base,cases:[],ok:false};
  const persist=()=>writeAtomic(path.join(base,'report.json'),JSON.stringify(report,null,2));
  try{
    if((await p.status()).ready)throw Error('8189 already occupied; leave it untouched');await p.start();const {nodes}=await p.inspect();
