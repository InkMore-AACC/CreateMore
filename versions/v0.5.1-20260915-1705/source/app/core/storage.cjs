...
const PROJECT_FILE = '项目.createmore';
...
function recoveryPath(target) {
  return path.join(path.dirname(target), '备份', path.basename(target) + '.previous');
}
...
async function atomicJSON(target, value, { backup = true } = {}) {
  ...
  try {
    ...
    if (backup) {
      const previous = recoveryPath(target);
      await noLinks(previous);
      await directory(path.dirname(previous), true);
      try {
        // Never replace the usable recovery file with a corrupt interrupted main file.
        JSON.parse(await fs.readFile(target, 'utf8'));
        const previousTemp = previous + '.tmp-' + id();
        try { await fs.copyFile(target, previousTemp); await fs.rename(previousTemp, previous); }
        finally { await fs.unlink(previousTemp).catch(err => { if (err.code !== 'ENOENT') throw err; }); }
...
async function readJSON(target, validate = () => true) {
  ...
  try { return { value: await parse(target), recovered: false }; }
    ...
    for (const backup of [recoveryPath(target), target + '.previous']) {
      try { return { value: await parse(backup), recovered: true, recoveryReason: primaryError.message }; } catch {}
    }
    throw primaryError;
  }
}
...
function validProject(value) {
  return value?.format === 'createmore-project' && value.version === 2 && typeof value.id === 'string' && typeof value.name === 'string' && Array.isArray(value.canvases) && value.canvases.every(canvas => canvas && typeof canvas.id === 'string' && typeof canvas.name === 'string' && typeof canvas.directory === 'string') && new Set(value.canvases.map(canvas => canvas.id)).size === value.canvases.length && new Set(value.canvases.map(canvas => canvas.directory.toLowerCase())).size === value.canvases.length;
}
function validCanvas(value) { return value?.format === 'createmore-canvas' && value.version === 2 && typeof value.id === 'string' && validState(value.state); }
...
async function copyTreeSafe(source, destination, { sanitize = false } = {}) {
  ...
  for (const entry of await fs.readdir(source, { withFileTypes: true })) {
    ...
    if (sanitize && (entry.name==='备份' || entry.name.startsWith('.') || /(?:\.previous|\.tmp-[\w-]+)$/.test(entry.name) || /^(auth\.json|credentials(?:\.json)?|connections\.json|secrets(?:\.json)?)$/i.test(entry.name))) continue;
...
class ProjectStore {
  ...
  async documentPath(dir, fallback) {
    await directory(dir);
    let names = (await fs.readdir(dir)).filter(name => name.toLowerCase().endsWith('.createmore'));
    if(!names.length){const backupDir=path.join(dir,'备份');await noLinks(backupDir);try{names=(await fs.readdir(backupDir)).filter(name=>name.toLowerCase().endsWith('.createmore.previous')).map(name=>name.slice(0,-9));}catch(err){if(err.code!=='ENOENT')throw err;}}
    if (names.length > 1) throw error('AMBIGUOUS_DOCUMENT', '同一目录内有多个工程文件，请把副本放进独立项目文件夹。');
    return child(dir, names[0] || fallback);
  }

  async _project(projectDir) {
    projectDir = await directory(projectDir);
    const file = await this.documentPath(projectDir, PROJECT_FILE);
    const result = await readJSON(file, validProject);
    for (const canvas of result.value.canvases) { validateName(canvas.directory); await noLinks(child(projectDir, canvas.directory)); }
    return { projectDir, file, ...result };
  }
  ...
  async _canvas(projectDir, canvasId) {
    ...
    const file = await this.documentPath(canvasDir, '画布.createmore');
    const result = await readJSON(file, value => validCanvas(value) && value.id === canvasId);
    return { project, entry, canvasDir, file, ...result };
  }
  ...
  async createProject(projectDir, name) {
    ...
    return exclusive(path.join(projectDir, PROJECT_FILE), async () => {
      if ((await fs.readdir(projectDir)).length) throw error('PROJECT_EXISTS', '请为新工程选择空文件夹，不会覆盖已有文件。');
      const manifest = { format: 'createmore-project', version: 2, id: id(), name, createdAt: now(), updatedAt: now(), canvases: [] };
  ...
  async resolveDocument(file) {
    if (typeof file !== 'string' || path.extname(file).toLowerCase() !== '.createmore') throw error('INVALID_DOCUMENT', '请选择 .createmore 工程文件；不支持旧 JSON 工程。');
    const absolute = (await regularFile(file)).absolute;
    const document = (await readJSON(absolute, value => validProject(value) || validCanvas(value))).value;
    const parent = path.dirname(absolute);
    const projectDir = validProject(document) ? parent : path.dirname(parent);
    const project = await this._project(projectDir);
    const expected = validProject(document) ? project.file : await this.documentPath(parent, '画布.createmore');
    if (absolute.toLowerCase() !== expected.toLowerCase()) throw error('INVALID_DOCUMENT', '工程文件不匹配。');
    if (validProject(document)) return { projectDir, canvasId: null };
    const entry = project.value.canvases.find(item => item.id === document.id && path.resolve(child(projectDir, item.directory)).toLowerCase() === parent.toLowerCase());
    if (!entry) throw error('CANVAS_NOT_FOUND', '画布与所在项目不匹配，请保留完整的项目文件夹。');
    return { projectDir, canvasId: entry.id };
  }
  ...
  async listRecent() {
    ...
    return Promise.all(records.map(async record => ({ ...record, exists: await this._project(record.projectDir).then(() => true, () => false) })));
  }
  ...
  async createCanvas(projectDir, name, state) {
    ...
    return exclusive(path.join(projectDir, PROJECT_FILE), async () => {
      ...
      const document = { format: 'createmore-canvas', version: 2, id: id(), name, createdAt: now(), updatedAt: now(), autosaveHash: hash(state), state: clone(state) };
      await atomicJSON(child(canvasDir, '画布.createmore'), document, { backup: false });
      ...
      await atomicJSON(project.file, project.value);
  ...
  async saveCanvas(projectDir, canvasId, state, { autosave = false, maxSnapshots } = {}) {
    ...
    return exclusive(path.resolve(projectDir) + ':' + canvasId, async () => {
      ...
      await atomicJSON(canvas.file, document);
      await exclusive(path.join(canvas.project.projectDir, PROJECT_FILE), async () => {
        ...
        await atomicJSON(current.file, current.value);
      });
      if (autosavePath) await this._rotateAutosaves(canvas.canvasDir, retention);
      return { saved: true, canvasId, path: canvas.file, autosavePath, unchanged };
    });
  }
  ...
  async relinkAsset(projectDir, canvasId, assetId, newFilePath, { batch = false } = {}) {
    ...
    return exclusive(path.resolve(projectDir) + ':' + canvasId, async () => {
      ...
      await atomicJSON(await this.documentPath(canvas.canvasDir, '画布.createmore'), document);
  ...
  async _package(projectDir, canvasId, destination) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        await atomicJSON(child(output, '画布.createmore'), document, { backup: false });
