...
test('focus cards lock the crop ratio and direct marking uses the picture instead of an annotation dialog',()=>{const uri='createmore-media://asset/test',node=normal('focus','image',{asset:uri,assetId:'asset',cardKind:'generator',ratio:'337:191',customWidth:337,customHeight:191,focusRegion:{x:1,y:2,cropWidth:337,cropHeight:191},objectMarks:[{id:'mark-one',name:'红色杯子',x:.5,y:.5,bbox:{x:.35,y:.25,width:.3,height:.5}}],prompt:'修改 📌[红色杯子](createmore-mark:mark-one)'}),ui=boot(state([node]));assert(ui.document.querySelector('.object-mark-box'));assert.match(ui.document.querySelector('.prompt-object-links').textContent,/红色杯子/);ui.click('.node[data-id="focus"] [data-action="spec"]');assert(ui.document.querySelector('.custom-ratio.locked'));assert.match(ui.document.querySelector('#panel-body').textContent,/固定遵循框选比例/);ui.click('[data-action="close-panel"]');ui.click('.node[data-id="focus"] [data-action="object-mark-mode"]');assert(ui.document.body.classList.contains('object-mark-picking'));assert.equal(ui.document.querySelector('#panel').hidden,true);});
test('prompt at-menu lists connected images only and numbers them in image order',()=>{const target=normal('target','image',{cardKind:'generator'}),picture=normal('picture','image',{asset:'createmore-media://asset/a',assetId:'a',cardKind:'asset'}),words=normal('words','text',{content:'说明',cardKind:'asset'}),snapshot=state([picture,words,target]);snapshot.edges=[{id:'e1',from:'words',to:'target'},{id:'e2',from:'picture',to:'target'}];const ui=boot(snapshot),field=ui.document.querySelector('.node[data-id="target"] [data-prop="prompt"]');field.value='请参考@';ui.send(field,'input');assert.match(ui.document.querySelector('#menu').textContent,/引用已连接的图片/);assert.match(ui.document.querySelector('#menu').textContent,/@1 picture/);assert.doesNotMatch(ui.document.querySelector('#menu').textContent,/words/);});
