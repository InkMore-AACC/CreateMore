...
test('service LAN downloads stable material copies, retains host external paths, and synchronizes new imports',async t=>{
  ...
  await a.saveCanvas();await b.saveCanvas();const stored=JSON.parse(await fs.readFile(path.join(b.current.canvasDir,'canvas.json'),'utf8'));assert.equal(stored.state.nodes[0].assetId,assetId);assert.ok(stored.state.assets.find(x=>x.id===assetId).path);
});
