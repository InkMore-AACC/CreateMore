...
(function () {
  'use strict';
  const {Graph,types,seed,blankShot,clone,ports,portFor,portPoint,nodeHeight,inViewport}=CanvasModel;
  ...
  function nodeTools(n){
    ...
    let tools='';
    if(displayType(n)==='image')tools=btn('tool','人像质感','person',attr+' data-tool="人像质感"')+btn('tool','多角度','eye',attr+' data-tool="多角度"')+btn('tool','打光','light',attr+' data-tool="打光"')+btn('tool','九宫格','grid',attr+' data-tool="九宫格"')+btn('tool','高清','expand',attr+' data-tool="高清"');
    ...
    else if(displayType(n)==='video')tools=btn('tool','逐帧拉片','table',attr+' data-tool="逐帧拉片"')+btn('tool','片段重拍','video',attr+' data-tool="片段重拍"');
  ...
  function nodeParams(n){
    const attr='data-node="'+n.id+'"',refs=state().edges.filter(e=>e.to===n.id).map(e=>graph.get(e.from)).filter(Boolean);
    const spec=n.type==='image'?btn('spec',(n.source==='Codex Image2'?esc(n.ratio)+' · ':'')+esc(n.count??1)+' 张',null,attr):'<span class="badge">'+(['video','audio','custom'].includes(n.type)?'按工作流参数':'参数见配置')+'</span>';
    return '<div class="node-params" data-owner="'+n.id+'"><div class="params-references">'+btn('reference','参考','plus',attr)+btn('tool','标记',null,attr+' data-tool="标记"')+btn('library-styles','风格','spark')+refs.map((r,i)=>'<button class="ref-tag" data-action="focus-node" data-node="'+r.id+'" title="'+esc(r.title)+'">'+(displayType(r)==='image'&&safeAsset(r.asset)?'<img src="'+esc(safeAsset(r.asset))+'" alt="'+esc(r.title)+'">':icon(types[r.type].icon))+'<b>'+(i+1)+'</b><span>'+esc(r.title)+'</span></button>').join('')+'</div><textarea aria-label="'+esc(n.title)+' 提示词" data-prop="prompt" placeholder="描述你的创作内容，@ 引用素材">'+esc(n.prompt)+'</textarea><div class="params-options"><select aria-label="生成来源" data-prop="source">'+sources(n).map(s=>'<option'+(n.source===s?' selected':'')+'>'+esc(s)+'</option>').join('')+'</select>'+spec+btn('generation','', 'up',attr+' class="generate-button" title="提交生成，状态以来源回执为准"')+'</div><details><summary>节点设置</summary>'+btn('resources','节点配置 ↗','settings')+'</details></div>';
  }
  ...
  function updateMediaVisibility(){
    ...
    for(const el of $('#nodes').querySelectorAll('[data-canvas-media]')){const n=nodes.get(el.dataset.canvasMedia),focused=ui.selected.size===1&&ui.selected.has(n?.id),visible=n&&!isHidden(n)&&inViewport(n,state().view,size,el.tagName==='IMG'?120:0);
      ...
      el.setAttribute('src',el.dataset.mediaSrc);
  ...
  function createMenu(x,y,source,outputId,atDrop=false){
    ...
    Object.entries(types).forEach(([type,t])=>{if(!source||graph.compatible(graph.get(source),{id:'new',type},{outputId}))html+='<button data-create="'+type+'">'+icon(t.icon)+t.name+(type==='custom'?'<small>工作流</small>':'')+'</button>';});
  ...
  function create(type,source=ui.source){
    ...
    try{const n=graph.create(type,x,y,source,source===ui.source?{outputId:ui.sourceOutputId}:{});ui.source=null;ui.sourceOutputId=undefined;dirty();locate(n.id);return n;}catch(e){toast(e.message);}
  }
  ...
  function action(name,b,event){
    ...
    switch(name){
      ...
      case 'spec':if(n?.type!=='image'){toast('此类型的时长、尺寸等使用当前工作流公开参数。');break;}ui.context=id;panel('图片输出设置','<form id="spec-form">'+(n.source==='Codex Image2'?'<div class="form-row"><label>画面比例</label><select name="ratio">'+['16:9','3:2','1:1','9:16'].map(v=>'<option'+(n.ratio===v?' selected':'')+'>'+v+'</option>').join('')+'</select></div><p class="muted">比例作为 Image2 的创作要求传入；实际像素尺寸以返回图片为准。</p>':'<p class="muted">尺寸、步数等仅使用当前来源公开的实际工作流参数；这里不覆盖未映射的参数。</p>')+'<div class="form-row"><label>图片数量</label><input type="number" name="count" value="'+esc(n.count??1)+'" min="1" max="8" required></div><div class="panel-footer">'+btn('cancel-config','取消')+'<button class="primary" type="submit">保存设置</button></div></form>','spec');break;
  ...
  document.addEventListener('submit',event=>{
    ...
    if(form.id==='spec-form'){const n=graph.get(ui.context),count=Number(values.get('count'));if(n?.type!=='image'||!Number.isInteger(count)||count<1||count>8){toast('图片数量必须是1至8的整数。');return;}graph.checkpoint();n.count=count;if(n.source==='Codex Image2'&&['16:9','3:2','1:1','9:16'].includes(values.get('ratio')))n.ratio=values.get('ratio');}
  ...
  document.addEventListener('pointerup',event=>{
    ...
    if(d.kind==='wire'){const target=d.moved?document.elementFromPoint(event.clientX,event.clientY)?.closest('[data-input]'):null;if(target){try{graph.connect(d.from,target.dataset.input,{inputId:target.dataset.inputId,outputId:d.outputId});dirty();}catch(e){toast(e.message);}}else createMenu(event.clientX,event.clientY,d.from,d.outputId,d.moved);ui.ignoreClickUntil=Date.now()+120;}
  ...
  let chromeFrame;
  function keepNodeControlsVisible(){
    if(chromeFrame)return;chromeFrame=requestAnimationFrame(()=>{chromeFrame=null;const bounds=$('#workspace').getBoundingClientRect(),k=state().view.k;
      for(const el of document.querySelectorAll('.node.selected')){const face=el.querySelector('.node-face').getBoundingClientRect(),visible=face.right>bounds.left&&face.left<bounds.right&&face.bottom>bounds.top&&face.top<bounds.bottom;
        const params=el.querySelector('.node-params');if(params){const bottom=bounds.bottom-100,topLimit=bounds.top+72;params.style.visibility=visible?'':'hidden';params.style.left='0px';params.style.top='calc(100% + 14px)';params.style.width=Math.min(Math.max(el.offsetWidth*k,620),bounds.width-32)+'px';params.style.maxHeight=Math.max(120,bottom-topLimit)+'px';params.style.overflowY='auto';params.style.transformOrigin='top left';params.style.transform='scale('+(1/k)+')';params.style.left=(el.offsetWidth-parseFloat(params.style.width)/k)/2+'px';if(visible){const r=params.getBoundingClientRect();let left=r.left,top=r.top;
            if(r.bottom>bottom){if(face.right+24+r.width<bounds.right-16){left=face.right+24;top=face.top;}else if(face.left-24-r.width>bounds.left+16){left=face.left-24-r.width;top=face.top;}else top=bottom-r.height;}
            left=Math.max(bounds.left+16,Math.min(left,bounds.right-16-r.width));top=Math.max(topLimit,Math.min(top,bottom-r.height));params.style.left=(left-face.left)/k+'px';params.style.top=(top-face.top)/k+'px';}}
      }
    });
  }
  window.addEventListener('canvas-render',keepNodeControlsVisible);
  document.addEventListener('pointerup',keepNodeControlsVisible);
  document.addEventListener('wheel',keepNodeControlsVisible,{passive:true});
  window.addEventListener('resize',()=>{renderView();keepNodeControlsVisible();});
  ...
  window.CreateMoreCanvas={graph,ui,state,render,renderNodes,renderSide,renderSelection,renderView,updateMediaVisibility,selection,locate,fit,create,panel,closePanel,closeTransient,showMenu,nav,btn,icon,esc,toast,dirty,safeAsset,script,downloadText,action,blankShot,types,clone};
  timer();
})();
