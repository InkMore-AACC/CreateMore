...
function bindingRoute(n,c){const incoming=(c?.state?.edges||[]).filter(e=>e.to===n.id).map(e=>(c.state.nodes||[]).find(x=>x.id===e.from)).filter(Boolean),kind=x=>x.type==='custom'?(x.outputType||'custom'):x.type,images=incoming.filter(x=>kind(x)==='image').length,audio=incoming.filter(x=>kind(x)==='audio').length;if(n.type==='image')return images>1?'multiImage':images===1?'singleImage':'text';if(n.type==='video')return images>1?'multiImage':images===1?'singleImage':'text';if(n.type==='audio')return audio?'singleAudio':'text';return 'text';}
...
class CreateMoreService extends EventEmitter {
  ...
  newNode(type,fields={},c=this.current){const t=types[type];return {id:id('n'),type,title:t.name,x:120+(c?.state.nodes.length||0)%4*90,y:150+(c?.state.nodes.length||0)%4*80,w:t.w,h:t.h,owner:this.identity.id,prompt:'',content:'',source:t.sources[0],ratio:'16:9',quality:'1K',count:1,cardKind:['style','character'].includes(type)?'asset':'generator',shots:type==='script'?[blankShot()]:[],...fields};}
  async references(n,c,workflow){return inputs.references(this,n,c,workflow);}
  async buildSnapshot(nodeId,overrides={},c=this.requireCurrent(),{skipLocalWorkflow=false}={}){const n=c.state.nodes.find(n=>n.id===nodeId);if(!n)fail('节点不存在');if(n.owner!==this.identity.id)fail('只能生成到自己的节点');if(n.cardKind==='asset')fail('普通素材卡片不执行生成；请创建对应的生成卡片','CAPABILITY_MISMATCH');const provider=overrides.provider||(n.source==='外部 API'?this.settings.externalProvider:SOURCES[n.source]);if(!provider)fail('请选择有效生成来源');
