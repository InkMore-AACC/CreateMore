...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    const reuse=process.argv.includes('--reuse-user-backend');await service.hub.configure({comfyui:{...config,url:reuse?'http://127.0.0.1:8188':'http://127.0.0.1:8189'}});
    const p=service.hub.providers.comfyui,prior=await p.status();if(reuse?(!prior.ready||prior.running||prior.pending):prior.ready)throw new Error('验收需要空闲的已授权后台，未启动或打断其他任务');
    const fixture=JSON.parse(await fs.readFile(path.join(root,'testing-output/local-cards-1788864300593/report.json'),'utf8')),source={};
    ...
    if(process.argv.includes('--speech')){
      const reference=JSON.parse(await fs.readFile(path.join(root,'testing-output/qwen-speech-1788862113795/report.json'),'utf8')).cases[0].result.outputs[0].path;
    ...
    for(const record of records){const item={id:record.id,name:record.name,mediaType:record.mediaType,workflowId:record.workflowId,startedAt:new Date().toISOString(),ok:false};report.cases.push(item);const began=Date.now();
      try{
        ...
        const beforeSelection=service.queue.tasks.length;await click(selector);
        const draft=await until(()=>run(id=>{const n=CreateMoreCanvas.state().nodes.find(n=>n.toolResource?.id===id&&!n.assetId);return n|| (CreateMoreDesktop.app.toolId===id&&!!document.querySelector('[data-desktop-form="tool"]')?.getClientRects().length?'form':null);},record.id),'当前工具卡片或专用表单');
        assert.equal(service.queue.tasks.length,beforeSelection,'选择工具不应启动生成');const generic=record.workflowId==='default:image-zimage-edit';
        if(draft!=='form'){
          await run(id=>{const C=CreateMoreCanvas;C.state().view={x:0,y:0,k:1};const n=C.graph.get(id);n.x=240;n.y=140;C.selection([id]);C.render();const field=document.querySelector('[data-owner="'+id+'"] [data-prop="source"]');field.value='本地 ComfyUI';field.dispatchEvent(new Event('input',{bubbles:true}));field.dispatchEvent(new Event('change',{bubbles:true}));},draft.id);
          await until(()=>run(id=>CreateMoreCanvas.graph.get(id)?.source==='本地 ComfyUI',draft.id),'选择本地来源');
        }else{
        ...
        const count=service.queue.tasks.length;await click(draft==='form'?'[data-desktop-form="tool"] button[type="submit"]':'[data-action="generation"][data-node="'+draft.id+'"].generate-button');
        if(generic){await until(()=>run(()=>document.body.textContent.includes('尚未接入已验收的专用本地工作流')),'专用能力拒绝提示');assert.equal(service.queue.tasks.length,count);item.status='blocked-unverified-specialized';item.ok=true;item.effectAccepted=false;}
          ...
          const results=service.current.state.nodes.filter(n=>n.originTaskId===task.id||n.id===task.nodeId&&n.taskId===task.id);assert.ok(results.length,'结果未回到卡片');if(draft!=='form')assert.equal(results[0].id,draft.id);item.results=results.map(n=>({id:n.id,type:n.type,shots:n.shots?.length}));item.outputs=[];
