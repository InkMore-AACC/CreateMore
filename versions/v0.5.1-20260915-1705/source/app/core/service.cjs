...
const objectMark=require('./object-mark.cjs');
...
const defaultCardBindings={
  image:{text:'default:image-zimage',singleImage:'default:image-qwen-edit-1',multiImage:'default:image-qwen-edit-2'},
  video:{text:'default:video-h3-turbo',singleImage:'default:video-h3-i2v',multiImage:''},
  audio:{text:'default:audio-qwen-emotion',singleAudio:'default:audio-qwen-clone'},
  text:{text:'default:text-qwen3'},script:{text:'default:text-qwen3'}
};
const defaultCardProfiles={image:{defaultSource:'Codex Image2',apiProvider:'openai-compatible'},video:{defaultSource:'外部 API',apiProvider:'openai-compatible'},audio:{defaultSource:'本地 ComfyUI',apiProvider:'openai-compatible'},text:{defaultSource:'Codex',apiProvider:'openai-compatible'},script:{defaultSource:'Codex',apiProvider:'openai-compatible'}};
function bindingRoute(n,c){const incoming=(c?.state?.edges||[]).filter(e=>e.to===n.id).map(e=>(c.state.nodes||[]).find(x=>x.id===e.from)).filter(Boolean),kind=x=>x.type==='custom'?(x.outputType||'custom'):x.type,markedSelf=n.type==='image'&&n.assetId&&objectMark.activeMarks(n).length?1:0,images=incoming.filter(x=>kind(x)==='image').length+markedSelf,audio=incoming.filter(x=>kind(x)==='audio').length;if(n.type==='image')return images>1?'multiImage':images===1?'singleImage':'text';if(n.type==='video')return images>1?'multiImage':images===1?'singleImage':'text';if(n.type==='audio')return audio?'singleAudio':'text';return 'text';}
function configuredBinding(settings,n,c){const route=bindingRoute(n,c);if(n.workflowId&&(n.type==='custom'||n.toolResource))return {binding:n.workflowId,route};const group=settings.cardBindings?.[n.type==='script'?'script':n.type];return {binding:group&&Object.hasOwn(group,route)?group[route]:settings.bindings?.[n.type==='script'?'text':n.type],route};}
...
class CreateMoreService extends EventEmitter {
  ...
  async init(){
    await fs.mkdir(this.dataDir,{recursive:true});this.settings=await readJSON(this.settingsFile,{saveMinutes:5,maxSnapshots:100,bindings:{image:'default:image-zimage',video:'default:video-h3-turbo',audio:'default:audio-h3-ambience',text:'default:text-qwen3'},tools:{'高清':{workflowId:'default:tool-image-upscale'},'片段重拍':{workflowId:'default:video-h3-i2v'}},externalProvider:'openai-compatible'});this.settings.cardBindings=Object.fromEntries(Object.entries(defaultCardBindings).map(([kind,routes])=>[kind,{...routes,...(this.settings.cardBindings?.[kind]||{})}]));this.settings.cardProfiles=Object.fromEntries(Object.entries(defaultCardProfiles).map(([kind,profile])=>[kind,{...profile,...(this.settings.cardProfiles?.[kind]||{})}]));this.settings.featureBindings={...(this.settings.featureBindings||{}),objectMark:{provider:'codex',workflowId:'default:tool-image-caption',...(this.settings.featureBindings?.objectMark||{})}};const {Diagnostics}=require('./diagnostics.cjs');this.diagnostics=await new Diagnostics({dataDir:this.dataDir}).init();this.logs=await this.diagnostics.list();
  ...
  async importAsset(file,{copy=false,targetId}={}){const c=this.requireCurrent();const target=targetId?c.state.nodes.find(n=>n.id===targetId):null;if(target&&target.owner!==this.identity.id)fail('不能修改他人的卡片');const content=inputs.isText(null,file)?await inputs.readText(file):'导入素材';const a=await this.store.importAsset(c.projectDir,c.id,file,{copy});c.state.assets=[...c.state.assets.filter(x=>x.id!==a.id),a];if(targetId){if(target){target.asset=a.asset;target.assetId=a.id;target.content=content;target.title=a.title;}}
    else{const n=this.newNode(a.type,{title:a.title,asset:a.asset,assetId:a.id,content,cardKind:'asset'});c.state.nodes.push(n);}c.dirty=true;c.revision++;await this.registerAssets(c);await this.syncSharedAssets?.(c);return this.view(c);}
  newNode(type,fields={},c=this.current){const t=types[type],preferred=this.settings?.cardProfiles?.[type]?.defaultSource,source=t.sources.includes(preferred)?preferred:t.sources[0];return {id:id('n'),type,title:t.name,x:120+(c?.state.nodes.length||0)%4*90,y:150+(c?.state.nodes.length||0)%4*80,w:t.w,h:t.h,owner:this.identity.id,prompt:'',content:'',source,ratio:'16:9',quality:'1K',count:1,cardKind:['style','character'].includes(type)?'asset':'generator',shots:type==='script'?[blankShot()]:[],...fields};}
  async references(n,c,workflow){return inputs.references(this,n,c,workflow);}
  async buildSnapshot(nodeId,overrides={},c=this.requireCurrent(),{skipLocalWorkflow=false}={}){const n=c.state.nodes.find(n=>n.id===nodeId);if(!n)fail('节点不存在');if(n.owner!==this.identity.id)fail('只能生成到自己的节点');if(n.cardKind==='asset')fail('普通素材卡片不执行生成；请创建对应的生成卡片','CAPABILITY_MISMATCH');const profile=this.settings.cardProfiles?.[n.type==='script'?'script':n.type]||{},provider=overrides.provider||(n.source==='外部 API'?(profile.apiProvider||this.settings.externalProvider):SOURCES[n.source]);if(!provider)fail('请选择有效生成来源');
    ...
    let workflow=skipLocalWorkflow?undefined:n.workflow;const selected=configuredBinding(this.settings,n,c),binding=selected.binding;if(!workflow&&binding&&provider==='comfyui'&&!skipLocalWorkflow)workflow=await this.workflowRead(binding,n.workflowVersion);if(provider==='comfyui'&&!workflow&&!skipLocalWorkflow)fail('当前输入类型没有绑定本地工作流：'+selected.route,'WORKFLOW_REQUIRED');
    const refs=await this.references(n,c,workflow),marks=objectMark.activeMarks(n);if(marks.length&&n.assetId&&!refs.some(ref=>ref.id===n.assetId)){refs.unshift({id:n.assetId,path:await this.assetPath(n.assetId,c),type:'image',title:n.title,selfMarked:true});}const styles=c.state.edges.filter(e=>e.to===n.id).map(e=>c.state.nodes.find(n=>n.id===e.from)).filter(x=>x?.type==='style').sort((a,b)=>(n.styleOrder||[]).indexOf(a.id)-(n.styleOrder||[]).indexOf(b.id)).filter(s=>!(n.disabledStyles||[]).includes(s.id));
    let prompt=[n.toolInstructions?.[provider]||n.toolInstructions?.default,objectMark.expandPrompt(n.prompt||n.content,n)].filter(Boolean).join('\n')+refs.filter(r=>r.text).map(r=>'\n\n参考文字 '+r.title+'：\n'+r.text).join('');if(n.type==='script')prompt+='\n请输出可编辑分镜表。仅返回JSON数组，每项包含duration（如5s）、description、prompt、size、camera、light、dialogue、sound。每项一个镜头，不输出Markdown围栏。';
    return {provider,kind,nodeId,canvasId:c.id,projectDir:c.projectDir,title:n.title,prompt,references:refs.filter(r=>r.path||r.inputId),workflow,workflowId:binding,workflowRoute:selected.route,workflowVersion:n.workflowVersion,parameters:{...n.parameters,ratio:n.ratio,quality:n.quality,count:Number(n.count??1),...overrides.parameters},styles:styles.map(s=>({id:s.id,title:s.title,content:s.content,skill:s.skill,provider:SOURCES[s.source]||'codex'})),outputDir};
  }
  ...
  async applyResult(task,result){if(task.snapshot?.remoteExecution)return;result=await inputs.hydrateText(result);return this.mutate(async()=>{
    ...
    if(n){n.taskId=task.id;delete n.error;if(result.text!=null){n.content=result.text;if(n.type==='script'){try{const parsed=JSON.parse(result.text.replace(/^```(?:json)?\s*|\s*```$/g,''));if(Array.isArray(parsed))n.shots=parsed;else if(Array.isArray(parsed.shots))n.shots=parsed.shots;}catch{n.content=result.text;}}}if(outputs.length){n.asset=outputs[0].asset;n.assetId=outputs[0].id;n.outputAssets=outputs.map(a=>a.id);n.outputBindings={};if(n.objectMarks?.length){n.prompt=objectMark.stripTokens(n.prompt);delete n.objectMarks;}for(const a of outputs)if(a.outputId){if(!n.outputBindings[a.outputId])n.outputBindings[a.outputId]={assetId:a.id,assetIds:[]};n.outputBindings[a.outputId].assetIds.push(a.id);}}n.generations=result.generations;n.generationErrors=result.generationErrors;n.styleTrace=result.styleTrace;n.lastResultAt=new Date().toISOString();}
    ...
    else if(task.snapshot.resultMode==='new'){const source=c.state.nodes.find(x=>x.id===task.snapshot.sourceNodeId);if(result.text&&!outputs.length){const next=this.newNode('text',{originTaskId:task.id,source:Object.keys(SOURCES).find(k=>SOURCES[k]===task.provider)||'本地媒体处理',title:task.title,content:result.text,cardKind:'asset',sourceNodeId:source?.id,x:(source?.x||100)+(source?.w||380)+140,y:source?.y||150},c);c.state.nodes.push(next);if(source)c.state.edges.push({id:id('e'),from:source.id,to:next.id});}for(const a of outputs){const content=inputs.isText(a.type,a.path)?await inputs.readText(await this.assetPath(a.id,c)):'处理结果';const next=this.newNode(a.type,{originTaskId:task.id,source:Object.keys(SOURCES).find(k=>SOURCES[k]===task.provider)||'本地媒体处理',title:task.title,asset:a.asset,assetId:a.id,content,cardKind:'asset',x:(source?.x||100)+(source?.w||380)+140,y:(source?.y||150)+outputs.indexOf(a)*330,sourceNodeId:source?.id},c);c.state.nodes.push(next);if(source)c.state.edges.push({id:id('e'),from:source.id,to:next.id});}}
  ...
  async call(method,args={}){
    ...
    if(method==='mark.identify')return objectMark.identify(this,args);
...
module.exports={CreateMoreService,emptyState,normalizeState,SOURCES,defaultCardBindings,bindingRoute,configuredBinding};
