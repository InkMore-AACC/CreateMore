...
(function () {
  ...
  function keys(){const entries=[['添加节点','双击空白 / 右键 / Tab'],['平移画布','空格 + 拖动 / 中键'],['缩放画布','滚轮'],['增加或移除选区','Shift + 点击'],['框选','拖动空白区域'],['复制 / 粘贴选区','Ctrl+C / Ctrl+V'],['复制到旁边','Ctrl+D / Alt + 拖动'],['撤销 / 重做','Ctrl+Z / Ctrl+Shift+Z'],['分组','Ctrl+G'],['删除卡片','Delete'],['保存画布','Ctrl+S'],['关闭浮窗','Esc']];panel('快捷键','<div class="shortcut-grid">'+entries.map(([a,b])=>'<div><span>'+a+'</span><kbd>'+b+'</kbd></div>').join('')+'</div>','keys');}
  ...
  function keepNodeControlsVisible(){
    if(chromeFrame)return;chromeFrame=requestAnimationFrame(()=>{chromeFrame=null;const bounds=$('#workspace').getBoundingClientRect(),k=state().view.k;
      for(const el of document.querySelectorAll('.node.selected')){const face=el.querySelector('.node-face').getBoundingClientRect(),visible=face.right>bounds.left&&face.left<bounds.right&&face.bottom>bounds.top&&face.top<bounds.bottom;
        const params=el.querySelector('.node-params');if(params){params.style.visibility=visible?'':'hidden';params.style.left='0px';params.style.top='calc(100% + 14px)';params.style.width=Math.min(el.offsetWidth,(bounds.width-32)/k)+'px';params.style.maxHeight=Math.max(120,(bounds.height-40)/k)+'px';params.style.overflowY='auto';if(visible){const r=params.getBoundingClientRect();let left=r.left,top=r.top;
            if(r.bottom>bounds.bottom-16){if(face.right+24+r.width<bounds.right-16){left=face.right+24;top=face.top;}else if(face.left-24-r.width>bounds.left+16){left=face.left-24-r.width;top=face.top;}else top=bounds.bottom-16-r.height;}
            left=Math.max(bounds.left+16,Math.min(left,bounds.right-16-r.width));top=Math.max(bounds.top+16,Math.min(top,bounds.bottom-16-r.height));params.style.left=(left-face.left)/k+'px';params.style.top=(top-face.top)/k+'px';}}
      }
    });
  }
